...
class MaskLoss(nn.Module):
    ...
    def _get_tgt_permutation_idx(self, indices, n_masks):
        ...
        # map_m = torch.zeros((torch.max(batch_idx) + 1, max(n_masks)))  #可能会导致过大的张量，超出计算机的内存限制
        map_m = torch.zeros((len(batch_idx), max(n_masks)), device=batch_idx.device)#修改后的张量穿创建
