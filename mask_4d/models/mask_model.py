...
# from pytorch_lightning.core.lightning import LightningModule#低于2.0版本的
from pytorch_lightning.core.module import LightningModule#高于2.0版本的内容
...
class Mask4D(LightningModule):
    ...
    def forward(self, x, track_ins):
        feats, coors, logits, query = self.backbone(x)  # 加入自定义的query
        outputs = self.decoder(feats, coors, track_ins)  # 代表decoder的三个输入，feats,coors,track_ins, 都是什么意思？
        # feats, coors, logits, query = cp(self.backbone, x)
        # outputs = cp(self.decoder, feats, coors, track_ins)
        return outputs, logits, query

    def training_step(self, x: dict, idx):
        # _, _, _, query_backbone, query_pe_backbone = self.backbone(x)  # 加入自定义的query
        track_ins = self.init_tracks()
        losses = {}
        for s_i in range(len(x["pt_coord"])):
            scan = {k: [v[s_i]] for k, v in x.items()}
            outputs, logits, backbone_query = self.forward(scan, track_ins)  # forward 函数修改输出变量
            ...
            # **********************************
            new_track_ins = self.init_tracks(backbone_query)
    ...
    def panoptic4d_inference(self, x):
        ...
        for i in range(len(x["pt_coord"])):
            if int(float(x["fname"][i].split("/")[-1][:-4])) == 0:
            # if x["fname"][i].split("/")[-1][:-4] == 0000000000.000000:
                self.track_ins = self.init_tracks()
                self.last_ins_id = 1
            ...
            outputs, logits, backbone_query = self.forward(scan, self.track_ins)  # forward 函数修改输出变量
    ...
    def instance_predictions(self, outputs, x):
        ...
        keep = labels.ne(num_classes) & torch.isin(labels, things_ids)  #torch1.9.0以上的版本才存在isin方法
    ...
    def init_tracks(self, query=None):#修改这个函数
        """
